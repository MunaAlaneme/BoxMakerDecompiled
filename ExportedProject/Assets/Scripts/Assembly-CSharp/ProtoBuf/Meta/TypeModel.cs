...
namespace ProtoBuf.Meta
{
	...
	{
		private sealed class DeserializeItemsIterator<T> : DeserializeItemsIterator, IEnumerator, IDisposable, IEnumerable, IEnumerator<T>, IEnumerable<T>
		{
		...
		private object DeserializeWithLengthPrefix(Stream source, object value, Type type, PrefixStyle style, int expectedField, Serializer.TypeResolver resolver, out int bytesRead, out bool haveObject, SerializationContext context)
		{
			...
			do
			{
				...
				int fieldNumber;
				int bytesRead2;
				num = ProtoReader.ReadLengthPrefix(source, flag, style, out fieldNumber, out bytesRead2);
				if (bytesRead2 == 0)
				{
					return value;
				}
			...
			try
			{
				...
				else if (!TryDeserializeAuxiliaryType(protoReader, DataFormat.Default, 1, type, ref value, true, false, true, false) && num != 0)
				{
					ThrowUnexpectedType(type);
				}
		...
		private bool TryDeserializeList(TypeModel model, ProtoReader reader, DataFormat format, int tag, Type listType, Type itemType, ref object value)
		{
			...
			while (TryDeserializeAuxiliaryType(reader, format, tag, itemType, ref value2, true, true, true, true))
			{
				result = true;
		...
		public object DeepClone(object value)
		{
			...
			int modelKey;
			if (GetWireType(Helpers.GetTypeCode(type), DataFormat.Default, ref type, out modelKey) != WireType.None && modelKey < 0)
			{
				return value;
			}
		...
		{
			string text = ((type != null) ? type.FullName : "(unknown)");
			if (type != null)
			{
		...
		private bool CanSerialize(Type type, bool allowBasic, bool allowContract, bool allowLists)
		{
			...
			if (protoTypeCode != 0 && protoTypeCode != ProtoTypeCode.Unknown)
			{
				return allowBasic;
			}
			int key = GetKey(ref type);
