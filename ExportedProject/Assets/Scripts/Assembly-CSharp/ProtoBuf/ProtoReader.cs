...
namespace ProtoBuf
{
	public sealed class ProtoReader : IDisposable
	{
		...
		{
			string text = ((type != null) ? type.FullName : "<null>");
		...
		public int ReadFieldHeader()
		{
			...
			uint value;
			if (TryReadUInt32Variant(out value) && value != 0)
			{
		...
		internal static Exception AddErrorData(Exception exception, ProtoReader source)
		{
			...
			{
				exception.Data.Add("protoSource", string.Format("tag={0}; wire-type={1}; offset={2}; depth={3}", source.fieldNumber, source.wireType, source.position, source.depth));
			}
			return exception;
		}
