...
{
	public struct BigInteger : IComparable<BigInteger>, IEquatable<BigInteger>, IComparable, IFormattable
	{
		...
		private static readonly uint[] _one = new uint[1] { 1u };
		...
		public BigInteger(double value)
		{
			...
			if (num == 0L)
			{
				...
				BigInteger bigInteger = ((!Negative(bytes)) ? One : MinusOne);
			...
			else
			{
				...
				num |= 0x10000000000000L;
				BigInteger bigInteger2 = num;
				bigInteger2 = ((num3 <= 1075) ? (bigInteger2 >> 1075 - num3) : (bigInteger2 << num3 - 1075));
		...
		public BigInteger(byte[] value)
		{
			...
			{
				uint num11 = (uint)(value[num10++] | (value[num10++] << 8) | (value[num10++] << 16) | (value[num10++] << 24));
				num9 = (ulong)num11 - (ulong)num8;
				num11 = (uint)num9;
				num8 = (uint)((int)(num9 >> 32) & 1);
				_data[k] = ~num11;
			}
			num6 = num & 3;
			if (num6 > 0)
			{
				...
				{
					num11 |= (uint)(value[num10++] << l * 8);
					num12 = (num12 << 8) | 0xFF;
				}
				num9 = num11 - num8;
				num11 = (uint)num9;
				num8 = (uint)((int)(num9 >> 32) & 1);
				_data[_data.Length - 1] = ~num11 & num12;
			}
			if (num8 == 0)
			{
				return;
		...
		public static BigInteger DivRem(BigInteger dividend, BigInteger divisor, out BigInteger remainder)
		{
			...
			uint[] q;
			uint[] r;
			DivModUnsigned(dividend._data, divisor._data, out q, out r);
			int num = r.Length - 1;
			while (num >= 0 && r[num] == 0)
			{
				num--;
		...
		{
			uint num = (uint)((ulong)_sign * 16843009uL);
			for (int i = 0; i < _data.Length; i++)
			{
				num ^= _data[i];
			}
		...
		public static double Log(BigInteger value, double baseValue)
		{
			...
			BigInteger one = One;
			long num7;
			for (num7 = num4; num7 > int.MaxValue; num7 -= int.MaxValue)
			{
				one <<= int.MaxValue;
			}
			one <<= (int)num7;
		...
		public static BigInteger Parse(string value, NumberStyles style, IFormatProvider provider)
		{
			BigInteger result;
			Exception exc;
			if (!Parse(value, false, style, provider, out result, out exc))
			{
				throw exc;
			}
			return result;
		...
		public byte[] ToByteArray()
		{
			...
			else
			{
				...
				num12 = (ulong)(~num2) + (ulong)num10;
				num11 = (uint)num12;
				if ((int)(num12 >> 32) == 0)
				{
					int num13 = FirstNonFFByte(num11);
					bool flag2 = (num11 & (1 << num13 * 8 - 1)) == 0;
					int num14 = num13 + (flag2 ? 1 : 0);
					if (num14 != num3)
					{
						array = Resize(array, num + num14);
					}
					while (num13-- > 0)
					{
						array[num8++] = (byte)num11;
		...
		{
			Exception exc;
			return Parse(value, true, style, cultureInfo, out result, out exc);
		}
		...
		private string ToString(uint radix, IFormatProvider provider)
		{
			...
			while (bigInteger != 0L)
			{
				...
				bigInteger = DivRem(bigInteger, radix, out remainder);
		...
		private static bool Parse(string s, bool tryParse, NumberStyles style, IFormatProvider provider, out BigInteger result, out Exception exc)
		{
			...
			for (; i < length; i++)
			{
				...
				if (c >= 'A' && c <= 'F')
				{
					byte b2 = (byte)(c - 65 + 10);
					bigInteger = bigInteger * num2 + b2;
					flag = true;
				}
		...
		public static explicit operator double(BigInteger value)
		{
			...
			{
				return (value._sign != -1) ? double.PositiveInfinity : double.NegativeInfinity;
			}
		}
		...
		public static explicit operator float(BigInteger value)
		{
			...
			{
				return (value._sign != -1) ? float.PositiveInfinity : float.NegativeInfinity;
			}
		}
		...
		public static BigInteger operator >>(BigInteger value, int shift)
		{
			...
			if (sign == -1)
			{
				...
				{
					BigInteger bigInteger3 = new BigInteger((short)sign, array);
