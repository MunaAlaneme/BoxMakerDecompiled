...
namespace Renci.SshNet.Security.Cryptography
{
	public sealed class RIPEMD160Hash : HashAlgorithm
	{
		...
		protected override byte[] HashFinal()
		{
			...
			ProcessBlock();
			UnpackWord(H0, array, 0);
			UnpackWord(H1, array, 4);
			UnpackWord(H2, array, 8);
			UnpackWord(H3, array, 12);
			UnpackWord(H4, array, 16);
			InternalInitialize();
			return array;
		}
		...
		{
			X[_offset++] = (input[inOff] & 0xFF) | ((input[inOff + 1] & 0xFF) << 8) | ((input[inOff + 2] & 0xFF) << 16) | ((input[inOff + 3] & 0xFF) << 24);
			if (_offset == 16)
			{
				ProcessBlock();
			}
