...
namespace SevenZip.CommandLineParser
{
	public class Parser
	{
		...
		private bool string_compare(string a, int index_a, string b, int index_b, int len)
		{
			for (int i = 0; i < len; i++)
			{
				int num = a[index_a + i];
		...
		private bool ParseString(string srcString, SwitchForm[] switchForms)
		{
			...
			while (num < length)
			{
				...
				for (int i = 0; i < _switches.Length; i++)
				{
					...
					if (length2 > num3 && num + length2 <= length && string_compare(switchForms[i].IDString, 0, srcString, num, length2))
					{
						num2 = i;
						num3 = length2;
					}
