...
public class UIInput : MonoBehaviour
{
	...
	public void UpdateLabel()
	{
		...
		else
		{
			...
			int num2 = (flag ? Mathf.Min(text2.Length, cursorPosition) : 0);
			string text4 = text2.Substring(0, num2);
			if (flag)
			{
				text4 += Input.compositionString;
			}
			text2 = text4 + text2.Substring(num2, text2.Length - num2);
			if (flag && label.overflowMethod == UILabel.Overflow.ClampContent && label.maxLineCount == 1)
			{
				int num3 = label.CalculateOffsetToFit(text2);
				if (num3 == 0)
				{
					mDrawStart = 0;
					RestoreLabelPivot();
				...
				{
					num3 = label.CalculateOffsetToFit(text2.Substring(0, num2));
					if (num3 > mDrawStart)
					{
						mDrawStart = num3;
						SetPivotToRight();
		...
		if (flag)
		{
			int num4 = mSelectionStart - mDrawStart;
			int num5 = mSelectionEnd - mDrawStart;
			if (mBlankTex == null)
			{
				...
				for (int k = 0; k < 2; k++)
				{
					for (int l = 0; l < 2; l++)
					{
						mBlankTex.SetPixel(l, k, Color.white);
					}
				}
				mBlankTex.Apply();
			}
			...
			if (num4 != num5)
			{
				label.PrintOverlay(num4, num5, mCaret.geometry, mHighlight.geometry, caretColor, selectionColor);
				mHighlight.enabled = mHighlight.geometry.hasVertices;
			}
			else
			{
				label.PrintOverlay(num4, num5, mCaret.geometry, null, caretColor, selectionColor);
				if (mHighlight != null)
				{
					mHighlight.enabled = false;
				}
	...
	{
		if (!base.enabled || validation == Validation.None)
		{
			return ch;
		}
